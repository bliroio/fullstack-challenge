...
The database is **not** seeded automatically on server start. To populate it with dummy data, run:
...
```bash
SEED_DB=true npm run seed:clear
```

> **Note:** Seeding requires `SEED_DB=true` and is blocked when `NODE_ENV=production`.
