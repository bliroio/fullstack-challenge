...
Defines the requirements for the `escapeRegExp` utility that escapes regex metacharacters for safe use in `new RegExp()` and MongoDB `$regex` queries, preventing ReDoS and regex injection attacks.
