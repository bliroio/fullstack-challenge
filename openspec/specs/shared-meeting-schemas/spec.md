...
### Requirement: Shared package exposes subpath exports
The `shared/package.json` SHALL declare an `exports` field mapping `"."` to `dist/index.js` (with types) and `"./schemas/*"` to `dist/schemas/*.js` (with types). This enables consumers to import specific schemas via `"shared/schemas/meeting"` without reaching into internal paths.

#### Scenario: Subpath import resolves to compiled output
- **WHEN** a consumer imports from `"shared/schemas/meeting"`
- **THEN** the import SHALL resolve to `shared/dist/schemas/meeting.js` (and `.d.ts` for types)

### Requirement: Client imports shared schemas as a workspace dependency
The `client/package.json` SHALL declare `"shared": "*"` as a dependency. The client's `tsconfig.json` SHALL NOT contain `@shared/*` path aliases or `"../shared/**/*.ts"` includes. Client code SHALL import `Meeting` (and other shared types) using `import type { Meeting } from "shared/schemas/meeting"`. The local `client/app/models/Meeting.ts` file SHALL NOT exist.
